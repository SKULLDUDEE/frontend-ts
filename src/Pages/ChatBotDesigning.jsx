...
export default function ChatBotDesigning() {
  ...
  const [customMessages, setCustomMessages] = useState(['Hi!', 'How can I help you?']);
  const [introductionPlaceholders, setIntroductionPlaceholders] = useState({
    name: 'Please Enter Your Name',
    phone: '+91',
    email: 'abc@gmail.com'
  });
  const [welcomeMessage, setWelcomeMessage] = useState('👋 Have any concerns tap me ');
  ...
  useEffect(() => {
    async function fetchConfig() {
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL}/chatbot/design`, {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
          credentials: 'include'
        });
  ...
  const handleSave = async () => {
    try {
      ...
      const saveRes = await fetch(`${import.meta.env.VITE_API_URL}/chatbot/design`, {
        method: 'PUT',
        headers: {
      ...
      if (saveRes.ok) {
        toast.success('Settings saved successfully!Thanks for your patience.');
      } else {
      ...
      <div className="settings-panel">
        ...
        <section className="setting-section">
          ...
          <div className="color-options">
            {['white', 'black', '#5b3333'].map(color => (
              <label
                key={color}
                className="color-circle"
                style={{
                  backgroundColor: color,
                  border: "1px solid rgba(22, 11, 11, 0.18)", 
                  borderRadius: "50%", 
                  padding: "5px", 
                }}
              >

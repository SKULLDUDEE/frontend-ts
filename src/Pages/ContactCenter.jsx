...
export default function ContactCenter() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        // Fetch user details
        const userRes = await fetch(`${import.meta.env.VITE_API_URL}/users/getUserDetails/${userId}`, {
          method: 'GET',
          credentials: 'include',
          headers: {
        ...
        // Fetch tickets
        const ticketsRes = await fetch(`${import.meta.env.VITE_API_URL}/tickets/${userId}`, {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
        ...
        // Check if ticketNumber exists in each ticket
        ...
        // Fetch team members
        if (user.teamId) {
          const teamRes = await fetch(`${import.meta.env.VITE_API_URL}/team/getTeamMembers/${user.teamId}`, {
            method: 'GET',
            credentials: 'include',
            headers: {
              'Content-Type': 'application/json',
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const msgsRes = await fetch(`${import.meta.env.VITE_API_URL}/tickets/${selectedTicket._id}/messages`, {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  useEffect(() => {
    ...
    const fetchUserDetails = async () => {
      try {
        const userRes = await fetch(`${import.meta.env.VITE_API_URL}/tickets/botuser/${selectedTicket.raisedBy}`, {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleAssign = async (e) => {
    ...
    try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/tickets/${selectedTicket._id}/assign`, {
            method: 'PATCH',
            headers: { 
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/tickets/${selectedTicket._id}/status`, {
        method: 'PATCH',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/tickets/${selectedTicket._id}/messages`, {
        method: 'POST',
    ...
    <div className="cc-container">
        ...
        <ul>
          {tickets.map(ticket => (
            <li key={ticket._id}
              ...
              <div>#{ticket.ticketNumber || ticket._id.slice(-6)}</div>
      ...
      {/* Section 2: Chat Window */}
        ...
        <header className="chat-header">
          {selectedTicket ? (
            <>
              <span>Ticket #{selectedTicket.ticketNumber || selectedTicket._id.slice(-6)}</span>

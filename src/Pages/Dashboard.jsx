...
export default function Dashboard() {
  ...
  const fetchTickets = async () => {
    let url;
    if (activeTab === 'all') {
      url = `${import.meta.env.VITE_API_URL}/tickets/${userId}`;
    } else {
      const statusParam = activeTab === 'resolved' ? 'resolved' : 'open';
      url = `${import.meta.env.VITE_API_URL}/tickets/${userId}/status/${statusParam}`;
    }
      ...
      <ul className="ticket-list">
        {tickets
          .filter(t => 
            ...
            (t.ticketNumber && t.ticketNumber.includes(searchTerm)) || 
            searchTerm === '')
          )
          .map(ticket => (
            <li key={ticket._id} className="ticket-item">
              <div className="ticket-main">
                ...
                />
                <div>
                  <div className="ticket-id">Ticket# {ticket.ticketNumber || ticket._id.slice(-6)}</div>
...
function TicketUser({ id }) {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL}/tickets/botuser/${id}`, {  
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
